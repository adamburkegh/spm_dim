...
public class MutateSingleNodeRandomTest {
	...
	public void genRandomChoice1() {
		...
		MutateSingleNodeRandom anr = new MutateSingleNodeRandom(registry);
	...
	private void hammerMutate(MutateSingleNodeRandom anr, ProbProcessTreeNode ppt) {
		ProbProcessTree current = ppt;
		for (int i=0; i<1000; i++) {
			try {
				current = anr.mutate(current);
			}catch (Exception e) {
				LOGGER.error(i + " :: " + ppt.toString(), e);
				fail(i + " :: " + ppt.toString());
			}
		}
	}
