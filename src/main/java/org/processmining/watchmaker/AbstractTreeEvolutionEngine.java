...
/**
 * This is a copy-paste fork of org.uncommons.watchmaker.framework.AbstractEvolutionEngine, as that
 * class declares a number of private fields unavailable to its descendants.  
 * 
 * @author burkeat
 *
 * @param <T>
 */
public abstract class AbstractTreeEvolutionEngine<T> implements EvolutionEngine<T> {

    // A single multi-threaded worker is shared among multiple evolution engine instances.
    private static FitnessEvaluationEvolutionWorker concurrentWorker = null;

    private final Set<EvolutionObserver<? super T>> observers = new CopyOnWriteArraySet<EvolutionObserver<? super T>>();

    // These fields are private in the original. A better workaround may be possible, but this was 
    // an improvement on the copy-paste fork of the entire framework. Or it may be a good example
    // of why private should be used sparingly in frameworks - you don't know the many future
    // use cases.
     ...
     */
    protected AbstractTreeEvolutionEngine(CandidateFactory<T> candidateFactory,
    ...
    protected List<EvaluatedCandidate<T>> evaluatePopulation(List<T> population)
    {
        ...
        else
        {
            ...
            try
            {
                ...
                for (T candidate : population)
                {
                    results.add(getSharedWorker().submit(new FitnessEvaluationEvolutionTask<T>(fitnessEvaluator,
                                                                                       candidate,
     ...
     */
    private static synchronized FitnessEvaluationEvolutionWorker getSharedWorker()
    {
        if (concurrentWorker == null)
        {
            concurrentWorker = new FitnessEvaluationEvolutionWorker();
        }
